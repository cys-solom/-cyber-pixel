...
async function renderSidebar() {
    ...
    const [oosRes, fullRes] = await Promise.all([
        adminApi('GET', '/out-of-stock'),
        adminApi('GET', '/full-activation')
    ]);
    const oos  = oosRes.out_of_stock  || false;
    const full = fullRes.enabled      || false;
    ...
    document.getElementById('sidebar-nav').innerHTML =
        ...
        `<div style="margin-top:12px;padding:0 4px;">
            <div style="font-size:9px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:1.2px;padding:0 8px;margin-bottom:6px;">Quick Controls</div>

            <!-- Out of Stock toggle -->
            <div class="sidebar-toggle-row" onclick="sidebarToggleOOS()" id="sidebar-oos-row">
                <div style="display:flex;align-items:center;gap:8px;flex:1;">
                    <span style="font-size:15px;">${oos ? '📦' : '✅'}</span>
                    <div>
                        <div style="font-size:12px;font-weight:700;color:${oos ? 'var(--red)' : 'var(--success)'}">${oos ? 'Out of Stock' : 'In Stock'}</div>
                        <div style="font-size:10px;color:var(--text-muted);">Activations</div>
                    </div>
                </div>
                <div class="sidebar-toggle-dot ${oos ? 'off' : 'on'}"></div>
            </div>

            <!-- Full Activation toggle -->
            <div class="sidebar-toggle-row" onclick="sidebarToggleFull()" id="sidebar-full-row">
                <div style="display:flex;align-items:center;gap:8px;flex:1;">
                    <span style="font-size:15px;">${full ? '⚡' : '⛔'}</span>
                    <div>
                        <div style="font-size:12px;font-weight:700;color:${full ? 'var(--cyan)' : 'var(--text-muted)'}">${full ? 'Full ON' : 'Full OFF'}</div>
                        <div style="font-size:10px;color:var(--text-muted);">Full Activation</div>
                    </div>
                </div>
                <div class="sidebar-toggle-dot ${full ? 'on' : 'off'}"></div>
            </div>
        </div>`;
}

async function sidebarToggleOOS() {
    const row = document.getElementById('sidebar-oos-row');
    if (row) row.style.opacity = '0.5';
    const cur = await adminApi('GET', '/out-of-stock');
    const newVal = !(cur.out_of_stock || false);
    const res = await adminApi('PUT', '/out-of-stock', { out_of_stock: newVal });
    if (res.success) {
        showToast(newVal ? '📦 Out of Stock — Activations stopped' : '✅ Back In Stock', newVal ? 'warning' : 'success');
        renderSidebar();
    } else {
...
async function sidebarToggleFull() {
    const row = document.getElementById('sidebar-full-row');
    if (row) row.style.opacity = '0.5';
    const cur = await adminApi('GET', '/full-activation');
    const newVal = !(cur.enabled || false);
    const res = await adminApi('PUT', '/full-activation', { enabled: newVal });
    if (res.success) {
        showToast(newVal ? '⚡ Full Activation enabled' : '⛔ Full Activation disabled', newVal ? 'success' : 'info');
        renderSidebar();
    } else {
        showToast('Failed to update', 'error');
        if (row) row.style.opacity = '1';
...
async function renderSettings(main) {
    const statusRes = await adminApi('GET', '/site-status');
    const currentStatus = statusRes.status || 'maintenance';
    const statusConfig = {
        open:        { label: '🟢 Open',         cls: 'btn-success', desc: 'Users can submit orders normally' },
        maintenance: { label: '🔧 Maintenance',   cls: 'btn-warning', desc: 'Orders blocked — service under maintenance' },
        closed:      { label: '🔒 Closed',         cls: 'btn-danger',  desc: 'Service permanently closed to users' }
    };
    main.innerHTML = `
    ...
    <div class="card" style="margin-bottom:20px;">
        ...
        <div class="card-body">
            <p style="font-size:13px;color:var(--text-secondary);margin-bottom:16px;">Control whether users can submit orders. Default is <strong style="color:var(--amber);">Maintenance</strong>.</p>
            <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:16px;">
                <button class="btn btn-sm ${currentStatus==='open'?'btn-success':'btn-ghost'}" id="status-btn-open" onclick="setSiteStatus('open')" style="flex:1;padding:14px;font-size:14px;font-weight:700;">🟢 Open<br><span style="font-size:10px;font-weight:400;opacity:.7;">Orders allowed</span></button>
                <button class="btn btn-sm ${currentStatus==='maintenance'?'btn-warning':'btn-ghost'}" id="status-btn-maintenance" onclick="setSiteStatus('maintenance')" style="flex:1;padding:14px;font-size:14px;font-weight:700;">🔧 Maintenance<br><span style="font-size:10px;font-weight:400;opacity:.7;">Orders blocked</span></button>
                <button class="btn btn-sm ${currentStatus==='closed'?'btn-danger':'btn-ghost'}" id="status-btn-closed" onclick="setSiteStatus('closed')" style="flex:1;padding:14px;font-size:14px;font-weight:700;">🔒 Closed<br><span style="font-size:10px;font-weight:400;opacity:.7;">Fully closed</span></button>
            </div>
            <div id="status-info" style="padding:10px 14px;border-radius:8px;font-size:13px;background:rgba(0,200,255,0.05);border:1px solid rgba(0,200,255,0.1);">
                Current: <strong id="status-label" style="color:${currentStatus==='open'?'var(--success)':currentStatus==='maintenance'?'var(--amber)':'var(--red)'}">${statusConfig[currentStatus].label}</strong> — ${statusConfig[currentStatus].desc}
            </div>
        </div>
    </div>
...
async function setSiteStatus(status) {
    const res = await adminApi('PUT', '/site-status', { status });
    if (res.success) {
        showToast(`Site set to ${status.toUpperCase()}`, status === 'open' ? 'success' : 'info');
        loadPage('settings');
    } else {
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input class="form-control" id="br-brand-name" value="${b.brand_name || 'Cyber Key'}" placeholder="Cyber Key">
                </div>
                <div class="form-group">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <div class="form-group">
                    ...
                    <input class="form-control" id="br-hero-title" value="${b.hero_title || 'Cyber Key'}" placeholder="Cyber Key" style="font-size:18px;font-weight:700;">
                </div>
                <div class="form-group">
