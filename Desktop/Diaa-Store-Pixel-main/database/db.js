...
async function seed() {
    ...
    if (!existingPricing) {
        await supabase.from('pricing').insert([
            ...
            { task_type: 'extract', points_cost: 2, source_credits: 1.0, label_en: 'Full Extract Link', label_ar: 'استخراج رابط كامل' },
            { task_type: 'purchase_link', points_cost: 2, source_credits: 1.0, label_en: 'Purchase Failed Link', label_ar: 'شراء رابط من طلب فاشل' }
        ]);
    ...
    // Default site status = maintenance
    const { data: existingStatus } = await supabase.from('settings').select('value').eq('key', 'site_status').single();
    if (!existingStatus) {
        await supabase.from('settings').insert({ key: 'site_status', value: 'maintenance' });
        console.log('[DB] Default site status set to maintenance');
    }
}
...
async function getAllActiveSourceCDKeys() {
    const { data } = await supabase.from('source_cdkeys').select('*').eq('is_active', true).order('id');
    return data || [];
}
...
module.exports = {
    ...
    // Source CDKeys
    getAllSourceCDKeys, getSourceCDKey, getActiveSourceCDKey, getAllActiveSourceCDKeys, getNextAvailableSource,
