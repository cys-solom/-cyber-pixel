...
app.get('/api/site-info', async (req, res) => {
    ...
    try {
        ...
        const outOfStock = oosSetting ? oosSetting.value === 'true' : false;
        const fullActSetting = await db.getSetting('full_activation_enabled');
        const fullActivationEnabled = fullActSetting ? fullActSetting.value === 'true' : false;
        res.json({
            credits: totalCredits,
            has_stock: totalCredits > 0 && !outOfStock,
            out_of_stock: outOfStock,
            full_activation_enabled: fullActivationEnabled,
...
if (process.env.VERCEL !== '1') {
    ...
    app.listen(PORT, () => {
        console.log(`\n🚀 Cyber Key running on http://localhost:${PORT}`);
